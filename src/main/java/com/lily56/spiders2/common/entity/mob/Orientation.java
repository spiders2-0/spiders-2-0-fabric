...
public class Orientation {
	...
	public Vec3d getGlobal(Vec3d local) {
		return this.localX.multiply(local.x).add(this.localY.multiply(local.y)).add(this.localZ.multiply(local.z));
	}
	...
	public Vec3d getGlobal(float yaw, float pitch) {
		...
		return this.localX.multiply(sy * cp).add(this.localY.multiply(sp)).add(this.localZ.multiply(cy * cp));
	}
